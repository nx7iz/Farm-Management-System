...
public:
  ...
  virtual void performTask(Animal *animals[], int animalCount, Crop *wheat, int &wheatDaysLeft,
                           Crop *corn, int &cornDaysLeft, int &wheatStock, int &cornStock) = 0;
...
public:
  void performTask(Animal *animals[], int animalCount, Crop *, int &, Crop *, int &, int &, int &) override
  {
    if (animalCount == 0)
    {
      cout << "No animals on the farm to feed.\n";
      return;
    }

    cout << "Animal Caretaker is feeding all animals...\n";
...
{
public:
  void performTask(Animal *[], int,
                   Crop *wheat, int &wheatDaysLeft,
                   Crop *corn, int &cornDaysLeft,
                   int &, int &) override
  {
    cout << "Field Worker is tending the fields...\n";

    if (wheatDaysLeft > 0)
    {
      wheatDaysLeft--;
      cout << "Wheat growth accelerated by 1 day.\n";
    }

    if (cornDaysLeft > 0)
    {
      cornDaysLeft--;
      cout << "Corn growth accelerated by 1 day.\n";
    }
  }
...
{
public:
  void performTask(Animal *[], int, Crop *wheat, int &wheatDaysLeft, Crop *corn, int &cornDaysLeft, int &wheatStock, int &cornStock) override
  {
    cout << "Harvester checking for ready crops...\n";
    ...
    if (wheatDaysLeft != 0 && cornDaysLeft != 0)
    {
      cout << "No crops ready for harvesting.\n";
    }
  }
...
int main()
{
  ...
  do
  {
    ...
    case 3:
      ...
      Worker *workers[3];
      workers[0] = new AnimalCaretaker();
      workers[1] = new FieldWorker();
      workers[2] = new Harvester();
      ...
      do
      {
        ...
        cout << "1. Animal Caretaker\n";
        cout << "2. Field Worker\n";
        cout << "3. Harvester\n";
        cout << "4. Back\n";
        cout << "Select which task to perform: ";
        cin >> workerChoice;

        if (workerChoice >= 1 && workerChoice <= 3)
        {
          workers[workerChoice - 1]->performTask(animals, animalCount, &wheat, wheatDaysLeft, &corn, cornDaysLeft, wheatStock, cornStock);
        }
    ...
    case 5:
      cout << "\nTotal workers created: " << Worker::getWorkerCount() << "\n";
      break;
